...
def gen_pbkdf2(
    password: bytes, salt: bytes, key_length: int, iterations: int = 10000
) -> bytes:
...
def gen_blake3_kdf(password: bytes, salt: bytes, key_length: int) -> bytes:
    ...
    return blake3.blake3(
        password + salt, derive_key_context="paradox-comparison-v1.0"
...
def _kdf_worker(args) -> bytes:
    ...
    if kdf_type == "paradox":
        ...
        return gen_pbkdf2(
            password_or_path, salt_or_nonce, key_length, extra.get("iterations", 10000)
        )
    elif kdf_type == "hkdf":
...
def main():
    parser = argparse.ArgumentParser(
        description="Paradox Cryptographic Comparison Study"
    )
    parser.add_argument(
        "--fast", action="store_true", help="Run a fast check (smaller sample sizes)"
    )
    parser.add_argument(
        "--full", action="store_true", help="Run the full evaluation (100k samples)"
    )
    parser.add_argument(
    ...
    if args.fast:
        ...
        p4_iterations = 3
        print(
            ">>> FULL MODE: Running full academic comparative benchmarks (this may take up to 4 minutes) ..."
        )
    else:
        ...
        for key_len in key_lengths:
            ...
            for i, psw in enumerate(passwords):
                if kdf == "paradox":
                    ...
                    key, _ = generate_key(
                        img, key_length=key_len, security_level="low", nonce=fixed_salt
                    )
                elif kdf == "pbkdf2":
    ...
    # Pre-select images
    avalanche_images = [
        use_image(image_paths[rng.randint(0, len(image_paths))])
        for _ in range(n_avalanche_runs)
    ]
    ...
    for kdf in kdfs_to_test:
        ...
        for i in range(n_avalanche_runs):
            ...
            if kdf == "paradox":
                k1, _ = generate_key(
                    img, key_length=32, security_level="low", nonce=fixed_salt
                )
            elif kdf == "pbkdf2":
                ...
                k1 = gen_argon2id(
                    pw, fixed_salt, 32, time_cost=1, memory_cost=8192, parallelism=1
                )
            elif kdf == "blake3":
            ...
            if kdf == "paradox":
                ...
                rc = rng.randint(0, 3)
                modified_pixels[ry, rx, rc] = (
                    int(modified_pixels[ry, rx, rc]) + 1
                ) % 256
                ...
                k2, _ = generate_key(
                    img_mod, key_length=32, security_level="low", nonce=fixed_salt
                )
                # Cleanup temp file
        ...
        print(
            f"    Mean bit difference: {p2_results[kdf]['mean']:.2f}% | Std Dev: {p2_results[kdf]['std']:.2f}%"
        )
    ...
    nonces = [
        hashlib.sha256(f"nonce_{j}".encode()).digest() for j in range(n_collision_keys)
    ]
    ...
    for kdf in kdfs_to_test:
        print(
            f"  Generating {n_collision_keys:,} keys for {kdf.upper()} collision check ..."
        )
        ...
        print(
            f"    Collisions: {collisions} | Rate: {rate:.6%} | Uniqueness: {uniqueness:.2f}% | Time: {elapsed:.2f}s"
        )
    ...
    for level in ["low", "medium", "high"]:
        ...
        for kdf in kdfs_to_test:
            ...
            _kdf_worker(
                (
                    kdf,
                    test_img.path if kdf == "paradox" else pw_perf,
                    test_nonce,
                    32,
                    extra,
                )
            ...
            p5_results[level][kdf] = {"peak_memory_mb": round(peak_mb, 4)}
            print(
                f"    Avg Time: {elapsed_avg*1000:.2f} ms | Keys/s: {keys_per_sec:.2f} | Peak Mem: {peak_mb:.3f} MB"
            )
    ...
    for size in p7_sizes:
        ...
        t0 = time.perf_counter()
        _, _ = generate_key(
            img_scale, key_length=32, security_level="low", nonce=fixed_salt
        )
        elapsed = time.perf_counter() - t0
    ...
    k_orig, _ = generate_key(
        img_orig, key_length=32, security_level="low", nonce=fixed_salt
    )
    p7_results = {}
    ...
    k_sp, _ = generate_key(
        img_sp, key_length=32, security_level="low", nonce=fixed_salt
    )
    ...
    k_scc, _ = generate_key(
        img_scc, key_length=32, security_level="low", nonce=fixed_salt
    )
    ...
    k_rot, _ = generate_key(
        img_rot, key_length=32, security_level="low", nonce=fixed_salt
    )
    ...
    k_crop, _ = generate_key(
        img_crop, key_length=32, security_level="low", nonce=fixed_salt
    )
    ...
    k_resize, _ = generate_key(
        img_resize, key_length=32, security_level="low", nonce=fixed_salt
    )
    ...
    print(
        f"  Generating sample of {sub_count:,} keys for exact pairwise Hamming distance distribution ..."
    )
    ...
    worker_inputs_p = [
        (
            "paradox",
            img_uniq.path,
            hashlib.sha256(f"u_{j}".encode()).digest(),
            32,
            {"security_level": "low"},
        )
    ...
    worker_inputs_h = [
        ("hkdf", pw_uniq, hashlib.sha256(f"u_{j}".encode()).digest(), 32, {})
        for j in range(sub_count)
    ]
    ...
    for _ in range(min(10000, sub_count * (sub_count - 1) // 2)):
        ...
        hd_p = sum(
            bin(x ^ y).count("1")
            for x, y in zip(paradox_keys[idx1], paradox_keys[idx2])
        )
        hd_h = sum(
            bin(x ^ y).count("1") for x, y in zip(hkdf_keys[idx1], hkdf_keys[idx2])
        )
    ...
    print(
        f"    Paradox key spacing  : Mean Hamming {p8_results['paradox']['mean_hamming']} | Std Dev {p8_results['paradox']['std_hamming']}"
    )
    print(
        f"    HKDF key spacing     : Mean Hamming {p8_results['hkdf']['mean_hamming']} | Std Dev {p8_results['hkdf']['std_hamming']}"
    )
    ...
    latencies = {
        kdf: [
            p4_results[lvl][kdf]["latency_s"] * 1000
    ...
    ax.bar(
        ...
        latencies["paradox"],
        width,
        label="Paradox (RVE-KDE)",
        color="#4e79a7",
    )
    ax.bar(
        x - width, latencies["pbkdf2"], width, label="PBKDF2-SHA256", color="#f28e2b"
    )
    ...
    ax.bar(
        x + 2 * width, latencies["blake3"], width, label="BLAKE3-KDF", color="#59a14f"
    )
    ...
    ax.plot(
        pixels_x, times_y, "o-", color="#4e79a7", linewidth=2, label="Paradox (LOW)"
    )
    ...
    ax.axvline(
        128, color="black", linestyle="--", linewidth=1.5, label="Ideal Mean (128 bits)"
    )
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["Metric Category", "KDF System", "Security Level / Parameter", "Value"]
        )
        ...
        for kdf in kdfs_to_test:
            for key_len in key_lengths:
                ...
                writer.writerow(
                    ["Entropy", kdf, f"{key_len*8}-bit key", res["entropy"]]
                )
                writer.writerow(
                    ["Chi-Square p-value", kdf, f"{key_len*8}-bit key", res["p_value"]]
                )

        # Avalanche rows
        for kdf in kdfs_to_test:
            writer.writerow(
                ["Avalanche Mean (%)", kdf, "256-bit key", p2_results[kdf]["mean"]]
            )
            writer.writerow(
                ["Avalanche Std Dev (%)", kdf, "256-bit key", p2_results[kdf]["std"]]
            )
        ...
        for kdf in kdfs_to_test:
            writer.writerow(
                [
                    "Collision Count",
                    kdf,
                    f"Batch of {n_collision_keys}",
                    p3_results[kdf]["collision_count"],
                ]
            )
        ...
        for lvl in ["low", "medium", "high"]:
            for kdf in kdfs_to_test:
                writer.writerow(
                    ["Latency (ms)", kdf, lvl, p4_results[lvl][kdf]["latency_s"] * 1000]
                )
                writer.writerow(
                    [
                        "Throughput (keys/sec)",
                        kdf,
                        lvl,
                        p4_results[lvl][kdf]["keys_per_sec"],
                    ]
                )
                writer.writerow(
                    [
                        ...
                        kdf,
                        lvl,
                        p5_results[lvl][kdf]["peak_memory_mb"],
                    ]
                )
    ...
    metrics_export = {
        "entropy_results": {
            kdf: {str(key_len * 8): p1_results[kdf][key_len] for key_len in key_lengths}
            for kdf in kdfs_to_test
        },
        "avalanche_results": {
            kdf: {
...
"""
    ...
    for p in [
        report_path,
        repo_dir / "paradox_comparisons" / "reports" / "comparison_report.md",
        Path(
            "/home/chirag/.gemini/antigravity-cli/brain/4655abb5-59ce-4897-be37-403501bd963f/comparison_report.md"
        ),
    ]:
