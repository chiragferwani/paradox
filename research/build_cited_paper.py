...
INTEXT = [
    ...
    ("PBKDF2 applies iterative hashing to slow down brute-force attacks", [5]),
    (
        "scrypt and Argon2id introduce memory-hardness to defeat GPU-based cracking",
        [4, 3],
    ),
    ("HKDF provides a clean extract-and-expand paradigm for deriving keys", [1, 2]),
    (
        "entropy of the resulting strings falls far short of what cryptographic security demands",
        [11],
    ),
    ("AES-256-GCM and ChaCha20-Poly1305 authenticated encryption.", [44, 43]),
    # I.II Problem Statement
    ("password entropy falls far short of", [11]),
    # I.III / Key Contributions — objectives
    ("fully compatible with AES-256-GCM and ChaCha20-Poly1305.", [44, 43]),
    # II.A Background KDFs
    ("HKDF was designed to solve a different problem entirely.", [1, 2]),
    (
        "TLS, QUIC, the Signal protocol, and many other systems need to derive multiple independent session keys",
        [1, 2, 8],
    ),
    ("HKDF does not compensate for entropy that was never there.", [1, 2]),
    ("scrypt remains a respected and widely deployed algorithm", [4]),
    ("Argon2 won the Password Hashing Competition in 2015", [3]),
    (
    ...
    ("BLAKE3 splits the input into chunks, hashes each chunk independently", [40]),
    ("BLAKE3 provides a key derivation mode as part of its specification", [40]),
    # II.A — BLAKE2 lineage
    (
    ...
    ("NIST-standardized pseudorandom number generation and PRNG standards.", [14]),
    ("OS-level entropy sources can have real-world vulnerabilities.", [15]),
    ("hardware entropy sources and the physics of true randomness generation.", [16]),
    ("statistical tests applied to random number generators.", [17]),
    ("entropy pooling; context for OS entropy pools.", [13]),
    # II.C Image entropy
    ("Seminal citation for visual cryptography", [18, 19, 20]),
    ("image hashing / image fingerprinting techniques.", [27]),
    ("chaotic-map image encryption; highlights nearest prior art", [22, 23]),
    ("theoretical limitations of digital piecewise-linear chaotic maps", [24]),
    ("steganographic pixel-walk use cases", [26]),
    # II.D Hash chains
    ("Foundational citation for hash chains in authentication", [28]),
    ("Primary citation for HMAC design", [29]),
    ("Merkle hash-tree / chained hash constructions", [30]),
    ("Supporting reference for MAC constructions built on hash chains.", [31]),
    ("fuzzy extractor / noisy key derivation constructions", [32]),
    # II.E Recursive Traversal
    ("Primary citation for the Hilbert space-filling curve", [33, 34]),
    ("locality-preservation properties of the Hilbert curve", [35]),
    ("adaptive pixel-walk algorithms in steganography", [36]),
    ...
    ("coordinate generation process may be represented", [47]),
    (
    ...
    ("local contrast and edge-structure context underpinning", [49]),
    # III.F Hash Chain Evolution
    ("avalanche propagation and HMAC-chain design principles", [29, 65]),
    ("formal avalanche criterion; grounds the avalanche propagation claim", [65]),
    # III.G Recursion Manager
    ("entropy amplification through iterated sponge-like constructions.", [39, 52]),
    # III.H Entropy Pool
    ("information-theoretic basis of entropy pool construction and aggregation.", [60]),
    # III.I KDF Compressor
    ("Primary citation for HKDF-SHA256 squeeze step.", [1, 2]),
    ("Primary citation for the BLAKE3-KDF alternative compression path.", [40]),
    # III.J Metadata
    (
    ...
    ("free-precomputation attacks; motivates the step/layer depth choices", [51]),
    (
        "HIGH-level memory and computation trade-offs relative to memory-hard functions.",
        [52],
    ),
    ("Argon2id's intentional 256 MB memory footprint as the reference benchmark", [3]),
    # V Implementation
    ("Paradox is implemented in Python, targeting version 3.9 and above.", [54]),
    ("NumPy array operations used throughout the pipeline", [55]),
    ("Pillow as the image decoding dependency.", [56]),
    ("SciPy statistical utilities used in diagnostic and validation modules.", [58]),
    ("Matplotlib as the visualization backend", [57]),
    ("OpenCV as an optional image-processing dependency.", [59]),
    ("AES-256-GCM encryption backend design and security properties.", [41, 45]),
    (
        "IETF-standardized ChaCha20-Poly1305 AEAD used in the second encryption backend.",
        [42, 43],
    ),
    ("SIFT as representative image feature-extraction literature", [50]),
    # VI Experimental Validation
    (
        "Foundational citation for the Shannon entropy metric applied to key output.",
        [11, 60],
    ),
    ("entropy computation formula and information-theoretic interpretation", [60]),
    (
    ...
    ("complementary randomness test suite.", [62]),
    ("third empirical randomness testing framework", [63]),
    ("formal avalanche criterion; grounds the ~50% bit-difference target.", [65]),
    ("Hamming distance as the pairwise key-difference metric.", [64]),
    ("collision resistance and bit-balance properties.", [66]),
    # VII Benchmark
    (
    ...
    ("Argon2id's intentional 256 MB memory target discussed in VII.D.", [3]),
    ("sustained-memory analysis; deepens the memory comparison discussion.", [87]),
    ("additional memory-hard KDF context broadening the benchmark comparison.", [69]),
    ("NumPy-based benchmark computation implementation.", [55]),
    ("SciPy statistical tools used in benchmark analysis.", [58]),
    # VIII Security Analysis
    ("Primary theoretical citation for nonce-misuse attacks", [80]),
    ("concrete example of nonce-reuse catastrophe in GCM", [75, 81]),
    ("multi-user nonce analysis; reinforces the severity", [81]),
    ("Supporting citation for nonce-misuse resilient encryption design.", [82]),
    ("formal threat-model framework (attacker capabilities, composability).", [79]),
    (
        "PRG-from-OWF foundations; relevant to the absence of a formal mixing proof.",
        [83],
    ),
    ("security definitions, threat models, and provable security gaps.", [84]),
    ("Foundational citation for timing side-channel attacks", [77]),
    ("power side-channel attacks; broadens the side-channel discussion.", [78]),
    ("GPU hash throughput data; quantifies the absence of hardware-hardness", [85]),
    ("Argon2id's formal memory-hardness against GPU attacks", [86, 87]),
    ("cautionary example of design-level backdoors", [76]),
    (
        "random oracle model; relevant to the absence of a formal walk-space mixing proof.",
        [100],
    ),
    # IX Future Work
    ("Primary citation for fractal geometry underlying Lévy-flight", [96, 97]),
    ("post-quantum threat landscape motivating lattice-based experiments.", [92, 93]),
    (
        "NIST PQC standardization process and rationale for quantum-resistant designs.",
        [93],
    ),
    ("leading NIST-selected lattice-based KEM", [94]),
    ("quantum adversary models applied to authentication", [95]),
    (
    ...
    # Discussion
    ("image-as-biometric key derivation extensions.", [88]),
    (
        "challenges of biometric-based key generation; parallels Paradox's image-driven approach.",
        [89],
    ),
    ("integration of biometric data into cryptographic systems.", [90]),
    ("template protection and reproducibility concerns in biometric crypto.", [91]),
    ("user-facing authentication reliability in biometric systems.", [98]),
]
...
make_style(
    "PAbstract", font_size=9, space_after=4, alignment=WD_ALIGN_PARAGRAPH.JUSTIFY
)
make_style(
    "PSecHead",
    font_size=10,
...
def is_roman_section(s):
    return bool(re.match(r"^(I{1,3}V?|VI{0,3}|IX|X{1,2})\.\s+[A-Z\u0026]", s.strip()))
...
while i < len(lines):
    ...
    if stripped.lower().startswith("keywords"):
        ...
        kw_text = (
            stripped[sep_idx + 1 :].strip() if sep_idx >= 0 else stripped[8:].strip()
        )
    ...
    # ----- TABLE headings -----
    if re.match(
        r"^TABLE\s+(I|II|III|IV|V|VI|VII|VIII|IX|X|\d+)", stripped, re.IGNORECASE
    ):
    ...
    if (
        ...
        or re.match(
            r"^\s*(framework_version|security_level|recursion_layers|nonce|kdf_algorithm|output_key_size|encryption_mode|image_hash)",
            stripped,
        )
        or stripped.startswith("paradox/")
